...
# Horizon settings (web UI): entrypoint=mikros_server/main.py:mcp, auth=off
...
[ -f mikros_server/main.py ] || fail "mikros_server/main.py not found"
ok "mikros_server/main.py found"
count=$(find mikros_server/workflows -name '*.yaml' 2>/dev/null | wc -l | tr -d ' ')
[ "$count" -gt 0 ] || fail "No workflow YAML files in mikros_server/workflows/"
ok "$count workflow(s) found"

if [[ "${1:-}" == "--inspect" ]]; then
    command -v fastmcp &>/dev/null || fail "fastmcp CLI not found"
    echo "" && fastmcp inspect mikros_server/main.py:mcp
fi
...
echo "  3. Select repo, set entrypoint: mikros_server/main.py:mcp"
