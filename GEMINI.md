...
> This file is the project's memory. Gemini CLI reads it on every session start. After `install.sh` runs, a compressed copy lives here and the human-readable original is at `GEMINI.original.md`. Edit `GEMINI.original.md` to make changes.
...
This project uses **mikros**. The workflow is:
