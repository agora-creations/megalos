...
4. **Self-host vs paid: explicit value asymmetry — ELEVATED PRIORITY.** What does paid megálos offer beyond what self-hosting offers? Operational convenience alone, or also features (observability, SSO, persistence guarantees, multi-replica, zero-deploy, managed updates)? Which of these are paid-only and which are self-host-capable. Deferred per operator answer; should be resolved before Phase I scaffold is drafted. *Priority elevated from v6 initial draft because consumer-subscription onramp removal (per Q2 resolution) shifts the paid-vs-self-host pitch from "we handle the OAuth dance" to "we offer operational maturity" — making the operational-maturity claim more load-bearing.*
