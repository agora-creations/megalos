...
**Status:** Superseded by [vision-v7](./2026-04-27-megalos-vision-v7.md) on 2026-04-27. Retained for historical reference; inline amendment-markers in §9 Q1 and §9 Q8 point readers at v7's brand inversion. Originally promoted to canonical on 2026-04-27 after OAuth-investigation amendment (§6.4, §9 Q2) and resolution of §9 Q9 (see [ADR-002](../adr/002-run-mode-session-persistence.md)) and §9 Q11 (see [ADR-003](../adr/003-phase-j-shipping-decision.md)).
...
1. **The naming question — RESOLVED at v6 ship; SUPERSEDED by [vision-v7](./2026-04-27-megalos-vision-v7.md) §6.2 + [ADR-008](../adr/008-brand-architecture.md) on 2026-04-27.** The v6 resolution is preserved below as historical record; the v7 amendment follows.

   *v6 resolution (preserved as historical record).* The curated catalog of MCP servers is **"the Library"** (capitalized when referring to the product surface; lowercase when referring to the general concept). The hosted chat UI product is **"megálos."** Under this resolution, "megálos" is the consumer-facing brand of the paid product specifically, and the underlying runtime is referred to as "the megálos runtime" or "`megalos-server`" in technical contexts where disambiguation matters. The Library is the OSS content layer; megálos is the surface that runs Library entries.

   *v6 implication for technical writing (also superseded).* Documentation that refers to "megálos" without qualification refers to the consumer-facing product from v6 forward. Documentation needing to refer to the runtime specifically uses "the megálos runtime" or the package name `megalos-server`. The Library entries (`agora-creations/megalos-writing`, etc.) keep their current `megalos-*` repo naming because the prefix denotes ecosystem membership, not product identity.

   ***Amendment 2026-04-27 — v7 brand inversion.*** Vision-v7 inverts the consumer-facing-brand resolution above. Under v7: **agorá** is the consumer-facing brand for both surfaces (catalog website at `agora-library.dev` and chat UI) and both deployments (self-host and hosted). **megálos** demotes to the runtime's technical name (`megalos-server` package; "the megálos runtime" in technical contexts); it is no longer consumer-facing. **mikrós** stays at the developer-tooling layer under existing naming. **The Library** terminology carries forward unchanged but lives in a single aggregator repo at `agora-creations/agora-library` per [ADR-009](../adr/009-repo-consolidation.md), with sibling folders (`writing/`, `analysis/`, `professional/`) that drop the `megalos-` prefix. See ADR-008 for the brand-architecture strategic reasoning and ADR-009 for the repo-consolidation operational reasoning.
...
8. **Consumer brand for the operator entity — DEFERRED at v6 ship; RESOLVED by [vision-v7](./2026-04-27-megalos-vision-v7.md) §6.2 + [ADR-008](../adr/008-brand-architecture.md) on 2026-04-27.** The v6 deferral is preserved below as historical record; the v7 resolution follows.

   *v6 deferral (preserved as historical record).* The operator is currently the `agora-creations` GitHub organization. Whether paid megálos carries that brand or a different one is a marketing question that intersects with question 1 (now resolved to "the Library" and "megálos") above. Resolution: likely `agora-creations` remains the GitHub-organization brand and "megálos" is the consumer-facing product brand; final naming for the legal/billing entity is deferred.

   ***Amendment 2026-04-27 — v7 resolves the consumer-brand question.*** Under v7: **agorá** is the consumer-facing brand. **`agora-creations`** remains the GitHub-organization brand (unchanged). The legal/billing-entity name is still operationally deferred but no longer strategically open — the consumer brand and the org brand are intentionally aligned (one accent and one hyphen apart, both Greek-rooted), signaling that the org is the operator behind agorá the product. See ADR-008 for strategic reasoning.
...
10. **Library entry versioning visible to the user — RESOLVED.** See [`docs/adr/005-library-entry-versioning-ux.md`](../adr/005-library-entry-versioning-ux.md). Decision: position (b) — opt-in upgrade with account-bound entry version, plus a narrow critical-fix carve-out (curator-marked CVE-class fixes auto-migrate; everything else prompts the user with [Upgrade]/[Stay] and defaults to Stay). Six commitments: account-bound version, non-blocking session-start prompt, critical-fix carve-out, version-binding log as first-class feature ((C) emission, (O) audit retention per ADR-004), bounded multi-version paid hosting (no indefinite history commitment), self-host pins by deployment cadence. Phase G UX work and Phase H session-store schema both inherit binding requirements from this ADR.
