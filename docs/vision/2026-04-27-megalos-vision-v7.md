# megálos vision-v7: agorá, the Library, and megálos
...
Vision-v6 re-centered the product narrative on two pillars — the Library (a curated catalog of MCP-server entries) and megálos (a hosted chat UI in which users pick Library entries and run them under BYOK inference). The technical thesis carried forward from v5; the commercial story sharpened.
...
**Reversal 1 — repo topology.** v6 inherited a multi-repo pattern from prior phases: each Library entry was its own repository under `agora-creations` (`megalos-writing`, `megalos-analysis`, `megalos-professional`, plus future additions). v7 consolidates these into a single aggregator repository with one folder per entry. Server-level catalog granularity from v6 §3.1 is preserved unchanged — one MCP server per Library entry, one entry per domain. Only the multi-repo *structural* pattern reverses.
...
**Addition — file-download consumption path.** v6 framed Library-entry consumption as MCP-connect-only: the user (or their AI agent) connects to a deployed MCP server endpoint and runs the entry's workflows. v7 adds an `npx`-style install path that drops a Library entry's YAML directly into the user's local `megalos-server` workflows directory, where it is loaded by the runtime exactly as a hand-authored workflow would be. The hosted-side equivalent is a "Use in agorá" affordance on the catalog website that adds the entry to the user's hosted agorá account. Two consumption paths, one Library, no schema change.

The conceptual analog explicitly validated by the operator: VoltAgent's `awesome-design-md` aggregator repo plus its `getdesign.md` website. One aggregator, library-per-folder, sibling website with library-per-page browsing, two install paths.
...
- **Three-dependency runtime.** `fastmcp`, `pyyaml`, `jsonschema`. Phase H pluggable backends live in extras. The Library, agorá, and the file-download path add no hard runtime dependency.
...
## 3. The product — agorá, the Library, and megálos
...
- **The catalog website** (`agora-library.dev` at v1 — see §6.4 for migration commitment). The discovery surface. A user lands here, browses Library entries by domain (writing, analysis, professional, future additions), reads what each does, and decides which to use. SEO matters because this is the entry-point surface; mostly-static pages with library-per-page browsing; SvelteKit + adapter-static is the v1 implementation per ADR-007.
- **The chat UI** (subdomain or path under `agora-library.dev` — exact hostname pinned at Phase G `/plan` time). The working surface. Users select a Library entry from the catalog, configure it lightly, supply their own LLM API key, and run the entry's workflows interactively. Plain Svelte SPA, static-bundle target per ADR-007. Two modes inside one UI: run mode (the mass surface, per v6 §3.2), and configure-mode-naive picker (per ADR-003 Clause 1).
...
### 3.2 The Library — the content layer

The Library is the curated catalog of megálos MCP-server entries. v6 §3.1 commits the granularity (one entry equals one MCP server) and the curation discipline (closed at launch under sole-author curation; opens with mikrós-driven scale). v7 preserves both unchanged. What v7 changes is repo topology.

**One aggregator repository.** The Library lives at `github.com/agora-creations/agora-library`. Each entry is a folder inside the repo: `writing/`, `analysis/`, `professional/`, plus future additions. The folder naming drops the `megalos-` prefix that prior repos carried — inside the aggregator the prefix is noise, and inside the post-inversion brand architecture the prefix references a layer (the runtime) the Library doesn't represent.

**Per-folder MCP endpoints retained.** Each folder deploys to its own MCP server endpoint. Hostname pattern at v1: `writing.agora-library.dev`, `analysis.agora-library.dev`, `professional.agora-library.dev` (per Q2 of the predecessor /discuss). Self-hosters configure their own hostnames; the pattern is the operator's hosted-deployment convention, not a runtime constraint.

**Per-folder versioning retained.** Each entry has its own release tag at the folder level (`writing-v0.6.0`, `analysis-v0.4.2`, `professional-v0.5.1`). ADR-005's account-bound version-binding model operates on per-folder versions; the aggregator-repo structure does not change ADR-005's commitments.

**OSS commitment unchanged.** Every entry is OSS at the source level under `agora-creations/agora-library`. v6 §6.6's OSS scope holds — runtime, Library, mikrós skill library, and agorá UI source are open in perpetuity; the operational layer is the operator's commercial code.
...
A Library entry is consumed in one of two ways. v6 named only the first; v7 adds the second.

**Path 1 — MCP-connect.** The user (or their AI agent) connects to a deployed MCP server endpoint and runs the entry's workflows. Hosted users connect to operator-deployed endpoints (`writing.agora-library.dev`, etc.); self-hosters deploy their own (or use someone else's). This is v6 §3.2's path, unchanged.

**Path 2 — File-download.** The user runs an `npx`-style command (or, for hosted users, clicks "Use in agorá") that drops the Library entry's YAML directly into their `megalos-server` workflows directory (or their hosted agorá account). The runtime loads it exactly as it would load any hand-authored workflow. No new schema, no special path in the runtime, no privileged API. The downloaded YAML is identical to what's served at the MCP endpoint — this is a distribution mechanism, not an alternative content shape.

The file-download path serves users who want to:

- **Run the entry locally without deploying an MCP server.** A Shape 1 or Shape 2 user developing in their own environment doesn't need a hosted MCP endpoint; the local `megalos-server` already runs the entry.
- **Customize the entry beyond what configure mode (Phase J, deferred per ADR-003) would allow.** A downloaded entry is a regular YAML workflow — the user can edit it freely, version-control it, fork it. The original entry on the catalog is unaffected.
- **Inspect the entry's contents before running.** The YAML is the source of truth; downloading it makes the contract visible without requiring an MCP connection first.

The hosted "Use in agorá" affordance is the same path served differently: the user's hosted agorá account gets the entry added, with the same opt-in upgrade and version-binding semantics ADR-005 commits.

### 3.4 megálos and mikrós — the technical layer

**megálos** is the runtime. The Python package is `megalos-server`; the runtime is "the megálos runtime" in technical contexts. It is not a consumer-facing brand. It is the implementation that runs Library entries, regardless of whether they're served via MCP-connect or loaded via file-download. Self-hosters install `pip install megalos-server`; hosted-product operators run the same package on their managed infrastructure. `megalos-server` is OSS in perpetuity per v6 §6.6.
...
| **Content layer** | the Library (`agora-library`) | Curated MCP-server entries with workflows. What agorá serves to users. |
...
A user encounters agorá and never needs to know about megálos or mikrós. A self-host operator encounters all three because they install the runtime that serves the consumer surface. A Shape 1 contributor authoring a Library entry encounters all three plus mikrós for AI assistance. Each layer addresses its own audience without bleeding into the others.
...
- **Shape 1 — Technical YAML authors.** Author Library candidates. Use shipped runtime and authoring DX. May self-host any combination of the Library and agorá. Do not pay for hosted agorá unless they value its operational maturity.
- **Shape 2 — Small teams and indie developers.** Build domain-specific workflows with mikrós assistance. May contribute Library candidates back. May self-host or use paid agorá.
- **Shape 3 — Enterprise self-hosters.** Self-host agorá plus Library on their own infrastructure. Phase H serves them.
- **Shape 4 — Hosted-plan customers.** Use paid agorá, pick Library entries, BYOK. The primary paying audience.
- **Shape 5 — Non-technical configurers.** Use paid agorá, pick Library entries, configure them lightly. Audience deferred per ADR-003 (Phase J shipping decision).

The shapes-as-gradient framing from v6 §4 holds — "build their own" (Shape 1) to "pick and configure" (Shape 5) — with agorá and the Library shared across all five.
...
- **Library granularity** (v6 §3.1). One entry equals one MCP server. The repo-topology reversal in v7 §6.1 does not change this.
- **Library curation discipline** (v6 §3.1). Closed at launch under sole-author curation; opens with mikrós-driven scale; mikrós-walked end-to-end and real-LLM-verified before shipping.
- **Library is OSS** (v6 §3.1, §6.6). Every entry is open-source under `agora-creations/agora-library`.
...
- **OSS commitment** (v6 §6.6). Runtime, Library, mikrós skill library, and consumer-surface UI source are OSS. Operational layer is the operator's commercial code.
...
The three current Library-entry repositories — `agora-creations/megalos-writing`, `-analysis`, `-professional` — collapse into one aggregator at `agora-creations/agora-library`. Each entry becomes a sibling folder: `writing/`, `analysis/`, `professional/`. Future entries are added as sibling folders without further repo proliferation.

**Server-level catalog granularity is preserved.** v6 §3.1 commits "one entry equals one MCP server"; v7 holds this commitment. Each folder deploys to its own MCP endpoint. The reversal targets only the multi-repo *structural* pattern, not the catalog granularity itself.

**Per-folder versioning retained.** Each entry tags its own releases (`writing-v0.6.0`, `analysis-v0.4.2`, etc.). ADR-005's account-bound entry-version binding operates on per-folder versions. The aggregator-repo structure does not collapse versioning to a single repo-level version.

**Per-folder MCP deployment retained.** Each folder deploys independently. The deployment hostname pattern is `<folder>.agora-library.dev` (per §6.4). Self-hosters configure their own hostnames.
...
- **Content:** the Library, hosted at `agora-creations/agora-library`. The repository name carries the agorá brand at the org-prefix level; folder names inside it (`writing/`, `analysis/`, `professional/`) are content-descriptive without brand prefix.
...
v6 §3.2 framed Library-entry consumption as MCP-connect-only — users connect to a deployed MCP server endpoint and run entries via FastMCP Client. v7 adds a second path: file-download.

**The npx-style install path.** A user runs a command that drops a Library entry's YAML into the user's local `megalos-server` workflows directory. The runtime loads it as a regular workflow on next startup (or via a hot-reload if MH4 is shipped). No new schema, no special runtime path, no privileged loading semantics. The downloaded YAML is byte-identical to the YAML the MCP endpoint would serve.

**The hosted-side "Use in agorá" affordance.** On the catalog website, each Library entry has a "Use in agorá" button alongside the existing MCP-connect option. Clicking it adds the entry to the user's hosted agorá account, applying ADR-005's account-bound version-binding from the moment of capture.
...
- File-download for "I want this entry in my own deployment, my own version control, my own modification path."
...
4. **Catalog website MVP.** Static site at `agora-library.dev`, library-per-page browsing, both consumption paths surfaced.
...
Restated for completeness. The runtime (`megalos-server`), the Library entries (`agora-creations/agora-library`), the mikrós skill library, and agorá's UI source are committed open-source in perpetuity. The operational layer — billing, auth, deployment automation, observability infrastructure, fleet management — is the operator's commercial code and is not open-source. ADR-004's (C)/(O)/(B) framework is the categorization discipline.
...
**Library curation (parallel sustained track).** Not a phase. Ongoing. Now operates on the consolidated `agora-library` aggregator with sibling folders rather than on three separate repos.
...
- **The chat UI's exact subdomain or path** (e.g., `app.agora-library.dev` vs `agora-library.dev/run`). Phase G `/plan` time.
...
- **The agora-library repo migration mechanics** (issue carryover, archive of old repos, redirect handling). Predecessor-work execution detail.
...
3. **The pricing model for agorá** (formerly "for megálos"). Per-server-hosted, per-active-Library-entry, flat-tier, freemium-with-metered-LLM-passthrough, hybrid. Phase I scoping question. Carries forward from v6 §9 Q3 unchanged in substance; only the surface name changed.

4. **Self-host vs paid: explicit value asymmetry — RESOLVED in v6 §9 Q4 → ADR-004.** Carries forward unchanged.

5. **The minimum Library size for agorá launch.** v6 §6.1's 5-to-10-domain-repos floor carries forward. Empirical question; resolved by user signal post-launch.

6. **The embedded agent's failure modes** (Phase J). Deferred with Phase J per ADR-003.

7. **Library quality enforcement at opened curation.** Future document. Closed curation is sufficient at v1.
...
10. **Library entry versioning visible to the user — RESOLVED in v6 §9 Q10 → ADR-005.** Carries forward unchanged. The aggregator-repo structure does not disturb ADR-005's per-folder versioning.
...
Vision-v7 reorganizes the consumer surface, the content layer, and the technical layer into three cleanly-named architectural levels: **agorá** (consumer), the **Library** (content, hosted at `agora-creations/agora-library`), and **megálos**/**mikrós** (technical and developer-tooling). The v6 product narrative — open-source content with paid infrastructure for operational maturity, BYOK inference in perpetuity, self-hosting first-class — carries forward unchanged.

The three changes from v6 are: repo consolidation (three Library-entry repos collapse to one aggregator with sibling folders, granularity preserved); brand inversion (agorá replaces megálos as consumer-facing brand for both surfaces and both deployments, megálos demotes to runtime technical name); file-download consumption path added alongside MCP-connect.
...
**Domain commitment.** v1 base domain is `agora-library.dev` (operator-registered, ~$15/yr). Bare-name `agora.{dev,run,tools}` is the preferred long-term shape; current registration status precludes acquisition at v7 ship. Future acquisition triggers a domain-migration amendment.

**Predecessor work** (brand resolution → this vision-v7 document → repo consolidation + per-folder deploy migration → catalog website MVP) lands before Phase H MH1 begins. Phase G's chat UI assumes the catalog at `agora-library.dev` already exists.
