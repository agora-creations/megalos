...
**`file_download_used` emitter location.** ADR-011 names `file_download_used` as one of the seven events. The catalog landing flow at MG1 S06 is where the file-download path enters the chat UI, but the actual download surface might live in MG2 (run-mode artifact downloads) or MG4 (artifact-gallery downloads). Default: emit from MG1 S06 (catalog landing flow's file-download intent) plus MG2 S05 (artifact-download from `generate_artifact` result). Confirm at MG1 S07's `/discuss`.
