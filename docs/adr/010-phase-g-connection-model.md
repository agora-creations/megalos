...
**Resolves:** Phase G roadmap scoping (`docs/vision/2026-04-27-phase-g-roadmap-scoping.md`) (P) /discuss C — connection model for the agorá chat UI, plus K (Workflows catalog discovery UI) which folds into C
**Related:** ADR-002 (run-mode session persistence, `002-run-mode-session-persistence.md` — backend mediates the resumption affordance); ADR-003 (Phase J shipping decision, `003-phase-j-shipping-decision.md` — backend instruments analytics from launch); ADR-004 (paid-vs-self-host value asymmetry, `004-paid-vs-self-host-value-asymmetry.md` — (C)/(O) split applied here); ADR-005 (collection versioning UX, `005-library-entry-versioning-ux.md` — backend resolves per-(user, collection) version-binding); ADR-006 (artifact retention decoupling, `006-artifact-retention-decoupling.md` — backend writes to `ArtifactStore`); ADR-007 (Phase G chat UI stack, `007-phase-g-chat-ui-stack.md` — single static bundle commits this ADR's "single chat UI codebase" requirement); ADR-008 (brand architecture, `008-brand-architecture.md` — agorá branding; commitment 2 governs the Library → Workflows brand-language shift this ADR's vocabulary follows); ADR-009 (repo consolidation, `009-repo-consolidation.md` — per-collection MCP endpoints; commitment 1 pins the aggregator name).

---

## Update (2026-04-27)

This ADR was originally drafted under v6 vocabulary ("Library entry", "Library catalog"). Per **ADR-008 commitment 2** (brand architecture, source-of-truth for the brand-language shift), the consumer-content brand renames to **Workflows** and the folder-unit "Library entry" renames to **collection**. This Update applies the renamed vocabulary throughout: "Library entry" → "collection"; "Library catalog" → "Workflows catalog"; references to per-entry endpoints → per-collection endpoints. The v1 hostname pattern migrates from `<folder>.agora-library.dev` to `<folder>.agora-workflows.dev` per v7 §6.4. The eight commitments, the Pattern B selection over Pattern A and Pattern C, and the MCP-over-HTTP refinement are unchanged; the rename is vocabulary-only.

---

## 1. Context

Vision-v7 §3.1 commits the agorá chat UI as a Svelte 5 SPA per ADR-007, served from `megalos-server` for self-host or from agorá's CDN for hosted. v7 §6.3 commits two consumption paths: MCP-connect (AI agents to per-collection endpoints) and file-download (collections dropped into local workflows directory). v7 §6.4 commits per-folder MCP endpoints under `<folder>.agora-workflows.dev`.
...
A focused /discuss session of 2026-04-27 walked three candidate patterns (browser-direct MCP to per-collection endpoints; backend-mediated single contract; hybrid split) against six criteria (single chat UI codebase per ADR-007; cross-cutting concerns in one place; (C)/(O) split per ADR-004; constitutional purity; self-host friction; operator role in hosted). Pattern A (browser-direct) was rejected on cross-cutting-concerns grounds and ADR-004 contradiction; Pattern C (hybrid) was rejected on ADR-007 single-bundle violation. Pattern B (backend-mediated single contract) won. A refinement — using MCP-over-HTTP as the contract rather than a separate custom HTTP API — collapses the "second interface to maintain" weakness and is adopted.

The K question (Workflows catalog discovery UI) was scoped in the Phase G roadmap scoping doc as (R), folding into whatever C settled. Under Pattern B, K resolves naturally: the catalog website (predecessor work, at `agora-workflows.dev`) is the discovery surface; the chat UI has a "switch collection" affordance backed by the backend's account-bound collection list rather than a duplicate in-product browser.
...
**4. Hosted backend is the operator's commercial agorá backend.** The browser hits agorá-hosted infrastructure (subdomain or path under `agora-workflows.dev`, pinned at /plan time per ADR-007 §3). The backend speaks MCP-over-HTTP to the chat UI on its outer surface; on its inner surface, it uses FastMCP Client (Python, server-side) to talk to per-collection endpoints (`writing.agora-workflows.dev`, etc.). The agorá backend is **(O) Operational layer** per ADR-004 commitment 7 — closed-source, paid-only, operator's commercial code. It proxies MCP traffic with cross-cutting concerns layered in.

**5. Per-collection MCP endpoints retained per v7 §6.4 and ADR-009.** The chat UI does not bypass them. AI-agent consumers (Claude Desktop, mikrós-driven agents, future adapters) continue connecting to `<folder>.agora-workflows.dev` directly via standard MCP transport. The hosted agorá backend proxies *to* these endpoints; self-host's local megalos-server *is* this kind of endpoint. The chat UI sits on top of the same surface AI agents use, just mediated through a backend that adds platform concerns.

**6. Cross-cutting concerns live in the backend layer, not in the chat UI.** ADR-002 session resumption, ADR-003 analytics instrumentation, ADR-005 collection-version-binding resolution, ADR-006 artifact storage — all backend-mediated. The chat UI surfaces UX for these (the resumption affordance, the version prompt, the artifact gallery) but does not implement the persistence, analytics, or version-resolution mechanics. The HTTP API contract surfaces only what the chat UI needs to drive the workflow loop and render its UX; the backend handles everything else under the hood.

**7. Multi-collection parallel sessions via browser tabs.** Each browser tab holds one MCP-over-HTTP session against one collection. The backend tracks sessions per-(user, collection) per ADR-002 §2 commitment 1's single-click resume framing (v6 prose: "single-click resume per Library entry"). The chat UI does not implement an in-product tab manager or multi-collection workspace — that's browser-tab-native, which keeps the UI's state model simple. Users wanting parallel sessions open additional tabs.

**8. Catalog discovery (K) lives at the website, not in the chat UI.** The agorá catalog at `agora-workflows.dev` is **not part of Phase G at all** — it is a predecessor-work deliverable per v7 §6.5, scoped and shipped before Phase G's MG1 begins. What folds from K into C is *only* the chat UI's in-product **"switch collection" affordance**: a small surface inside the chat UI that lets users move between collections within their account-bound collection list, where the collection list is **resolved by the backend per ADR-005** (account-bound, version-bound, accumulated via "Use in agorá" captures from the catalog website). The chat UI never needs its own catalog browser because (i) the catalog website is the discovery surface and (ii) the backend is the authority on which collections the user has access to. Users discover collections on the catalog; the chat UI's job is execution and resumption against the collections the backend says the user has, not browsing.
...
**MG3 — Catalog and picker** implements the in-chat-UI "switch collection" affordance per commitment 8. The picker surfaces the user's account-bound collection list (resolved by the backend); the discovery surface lives on the catalog website. Configure-mode-naive picker per ADR-003 Clause 1.
...
- A hosted agorá backend that exposes MCP-over-HTTP on its outer edge and uses FastMCP Client on its inner edge to proxy to per-collection endpoints. (O) per ADR-004 commitment 7. Closed-source operator's commercial code.
...
- Multi-collection sessions via browser tabs. The chat UI does not implement an in-product tab manager.
- Catalog discovery on the agorá catalog website (predecessor-work deliverable); chat UI has an in-product "switch collection" affordance only, backed by account-bound collection list.
- Per-collection MCP endpoints (per v7 §6.4 + ADR-009) continue to serve AI-agent consumers directly. The chat UI doesn't bypass them; the hosted backend proxies through them.
...
- Pattern A (browser-direct MCP to per-collection endpoints). Foreclosed structurally on cross-cutting-concerns grounds; reopening would require reopening ADR-002, ADR-003, ADR-005, and ADR-006 in concert.
...
- An in-product Workflows catalog browser inside the chat UI. The catalog website is the discovery surface; the chat UI has a "switch collection" affordance only.
- Browser-side direct connections from chat UI to per-collection endpoints in hosted. The agorá backend always sits in the path for hosted; this is positive structural commitment per commitment 4.
- An operator-bypass mode where users connect their hosted-tier chat UI to self-managed collection endpoints. Hosted means hosted; if a user wants direct-to-self-managed-endpoints, they self-host.
...
**The hosted agorá backend's MCP-over-HTTP proxy layer introduces unmanageable issues.** Browser → backend → per-collection-endpoint adds two MCP hops where Pattern A would have had one. If proxying surfaces issues — latency stack-up, capability-mismatch between MCP versions on browser and per-collection sides, session-state synchronization across the proxy boundary, streaming response coordination — the architecture's value proposition weakens. Mitigation: a Phase H MH2-adjacent sanity check verifies the proxy layer is buildable cleanly before MG3 (which assumes it). If proxying turns out to be substantially harder than expected, this ADR reopens — likely toward a thinner backend with browser-side direct connections for some paths, which would partially-restore Pattern A's tradeoffs.
...
- **T2 — Hosted-backend proxy issues surface.** Latency, capability-mismatch, session-state, or streaming coordination problems in the browser → backend → per-collection proxy layer. Reopen commitment 4 toward thinner backend.
...
- vision-v7 §3.2 (Workflows — content layer; per-folder MCP endpoints).
...
- vision-v7 §6.4 (external surface; per-folder endpoints under `<folder>.agora-workflows.dev`).
