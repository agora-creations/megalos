# ADR-005 — Collection versioning UX

**Status:** Accepted, 2026-04-27
**Supersedes:** none
**Governing vision:** [`2026-04-27-megalos-vision-v7.md`](../vision/2026-04-27-megalos-vision-v7.md)
**Resolves:** vision-v6 §9 Q10 (collection versioning visible to the user; v6 prose framed this as "Library entry versioning")
**Related:** ADR-001 (workflow versioning, `docs/adr/001-workflow-versioning.md` — pins within-session versioning, which this ADR's cross-session reasoning rests on); ADR-002 (run-mode session persistence, `docs/adr/002-run-mode-session-persistence.md` — the session-store interface this ADR extends with per-user collection-version binding); ADR-004 (paid-vs-self-host value asymmetry, `docs/adr/004-paid-vs-self-host-value-asymmetry.md` — the (C)/(O)/(B) framework this ADR applies); ADR-008 (brand architecture, `008-brand-architecture.md` — commitment 2 governs the Library → Workflows brand-language shift this ADR's vocabulary follows); Phase H roadmap MH2 (`SessionStore` interface).

---

## Update (2026-04-27)

This ADR was originally drafted as "Library entry versioning UX." Per **ADR-008 commitment 2** (brand architecture, source-of-truth for the brand-language shift), the consumer-content brand "Library" renames to **Workflows** and the folder-unit "Library entry" renames to **collection**. This Update applies the renamed vocabulary throughout the body: "Library entry" → "collection" in active prose, "the Library" → "Workflows" where the consumer-content brand is meant. Quoted v6 prose and historical citations retain "the Library" / "Library entry" framing as historical record. Commitments and reasoning are unchanged; the rename is vocabulary-only.

---

## 1. Context

Vision-v6 §3.1 names "Library entries" (now: collections) as the curated catalog: "Each entry is a single domain repository — `megalos-writing`, `megalos-analysis`, `megalos-professional`, plus future additions — bundling related workflows behind one MCP server with a single deploy story." Curation is closed at launch and opens with mikrós-driven scale; each collection ships only after the four-skill mikrós ladder has been walked end-to-end and after independent verification against a real LLM has confirmed workflow completion against spec.

That curation discipline implies deliberate, tested releases — but releases happen. A user who picks a collection today and returns next week may find that the curator has shipped a new tagged version. Workflow contents may have been improved, the workflow set may have changed (additions or removals), the schema version may have been bumped, the runtime version requirement may have moved. v6 §9 Q10 named the strategic question: what does the user experience when a collection updates upstream — silent migration, opt-in upgrade, or version pinning?

ADR-001 already pins the within-session answer: at session create time, the workflow's canonical fingerprint is snapshotted; mid-session changes to the underlying YAML produce a terminal `workflow_changed` envelope. So within a session, the workflow is pinned by construction. Q10 is the *cross-session* question: when user U starts a new session against collection E and E has been updated since U's last session, what does U experience?

Granularity note: "collection version" most naturally means the curator's tagged release of the collection's folder under the aggregator (e.g., `writing-v0.6.0`). The /discuss treats "collection was updated" as "curator shipped a new tagged release"; concrete deployment-model questions (multiple versions deployed in parallel? rolling upgrade?) are Phase G/I scoping work, not Q10's strategic answer.
...
**1. Cross-session collection version is account-bound.** A user's account remembers the last-used collection version per collection. New sessions resolve to that version unless the user takes action to change it. The binding is per-(user, collection) — a user can be on different versions of different collections simultaneously.

**2. New-version prompt at session start.** When a session against collection E starts and V_current(E) > V_user(E), the UI surfaces a non-blocking "newer version available" prompt with [Upgrade] / [Stay on V_user] options. **Default action: "Stay on V_user"** — no surprise behavior change without an explicit user click. The prompt is informational by default; it does not block session start.
...
- Phase H MH2 session-store schema accommodates per-user collection-version binding. Verified at MH2 `/discuss` alongside ADR-002's requirements.
- Collection version becomes a first-class concept — exposed in UI, logged in session metadata, persisted in account binding. Not just an internal release tag.
...
**Workflows curators ship far more often than v6 §3.1's deliberate-release discipline implies.** If mikrós-driven authoring scale produces curator iteration cadences in the dozens per year per collection, prompt fatigue from constant "upgrade available" nudges drives users toward (i) auto-accepting (effectively becoming silent migration), (ii) ignoring everything (running far-behind versions), or (iii) demanding pinning (push toward position (c) or (d)). The (b) default rests on curators shipping deliberately. If they don't, the framework needs revision toward (d) — add a per-user "always upgrade" / "never auto-prompt" toggle as opt-in evolution.
...
- **T2 — Workflows cadence outpaces curator discipline.** Quarterly review shows >12 tagged releases per collection per year. May indicate v6 §3.1 curation discipline isn't holding; revisit (b)'s assumption about deliberate releases.
...
- **T4 — Workflows opens to third-party submissions** (per v6 §3.1 mikrós-driven scale opening to third-party contributions). Curator discipline becomes harder to enforce uniformly; (b)'s assumption about deliberate releases weakens; may need per-collection curator-discipline metadata or a stricter default for less-vetted collections.
...
- vision-v6 §3.1 (Workflows curation discipline that this ADR's (b) default rests on; v6 prose: "the Library").
- vision-v6 §6.1 (v1 Workflows scale of 5–10 domain repos — bounds operational cost of commitment 5).
- vision-v6 §6.6 (OSS scope: runtime, Workflows, mikrós, UI source open; operational layer closed).
...
- vision-v7 §3.2 + §6.2 (Workflows brand inversion and per-collection vocabulary that this Update aligns with).
...
- ADR-002 (`docs/adr/002-run-mode-session-persistence.md`) — `SessionStore` interface that ADR-005 commitment 4 extends with per-user collection-version binding requirements.
...
- ADR-008 (`008-brand-architecture.md`) — commitment 2 is the source-of-truth for the Library → Workflows brand-language shift this ADR's Update applies.
- Phase H roadmap (`docs/vision/2026-04-22-phase-h-distribution-hardening-roadmap.md`) — MH2 session-store interface accommodates per-user collection-version binding.

---

*End of ADR-005. This ADR's commitments hold conditional on Workflows curators maintaining the discipline established in v6 §3.1 (deliberate, tested releases) and the curator-side critical-fix-marker discipline (commitment 3, used sparingly). If either lapses, the (b) default loses its load-bearing premise — prompt fatigue from over-frequent releases or trust collapse from over-marked releases both reopen this ADR.*
