...
# --- SKILL.md has no mikros-specific paths ---
assert_file_not_contains "$DIR/SKILL.md" ".mikros/STATE.md" "no .mikros/STATE.md in standalone SKILL.md"
...
assert_file_contains "$DIR/SKILL.md" "MIKROS_LOC_BUDGET" "mentions env var"
...
assert_file_contains "$DIR/scripts/loc-budget.sh" "MIKROS_LOC_BUDGET" "loc-budget.sh reads MIKROS_LOC_BUDGET"

# --- loc-budget.sh falls back to .mikros/STATE.md ---
assert_file_contains "$DIR/scripts/loc-budget.sh" ".mikros/STATE.md" "loc-budget.sh falls back to STATE.md"

# --- loc-budget.sh works with env var (functional test) ---
TESTS_RUN=$((TESTS_RUN + 1))
# Budget of 99999 should always pass
if ! MIKROS_LOC_BUDGET=99999 bash "$DIR/scripts/loc-budget.sh"; then
  TESTS_FAILED=$((TESTS_FAILED + 1))
  echo "FAIL: loc-budget.sh should exit 0 with large budget" >&2
fi

# --- anti-patterns.md has no mikros-specific references ---
assert_file_not_contains "$DIR/references/anti-patterns.md" ".mikros/" "anti-patterns.md has no .mikros/ paths"
