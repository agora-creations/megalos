...
class TestFullWorkflow:
    ...
    def test_happy_path(self):
        r = _call("start_workflow", {"workflow_type": "coding", "context": "build a CLI"})
        ...
        for i, step_id in enumerate(STEPS):
            r = _call("submit_step", {"session_id": sid, "step_id": step_id, "content": f"content-{step_id}"})
        ...
        # Verify final state
        st = _call("get_state", {"session_id": sid})
...
class TestGateEnforcement:
    ...
    def test_out_of_order_rejected(self):
        r = _call("start_workflow", {"workflow_type": "coding", "context": "test"})
        sid = r["session_id"]
        # Try submitting 'plan' when 'discuss' is current
        r = _call("submit_step", {"session_id": sid, "step_id": "plan", "content": "nope"})
        assert "error" in r
    ...
    def test_premature_artifact_rejected(self):
        r = _call("start_workflow", {"workflow_type": "coding", "context": "test"})
        sid = r["session_id"]
        r = _call("generate_artifact", {"session_id": sid})
        assert "error" in r
...
class TestEdgeCases:
    ...
    def test_unknown_workflow(self):
        r = _call("start_workflow", {"workflow_type": "nonexistent", "context": "x"})
        assert "error" in r
        assert "available_types" in r

    def test_invalid_session(self):
        r = _call("get_state", {"session_id": "bogus123"})
        assert "error" in r

    def test_get_guidelines(self):
        r = _call("start_workflow", {"workflow_type": "coding", "context": "test"})
        sid = r["session_id"]
        g = _call("get_guidelines", {"session_id": sid})
